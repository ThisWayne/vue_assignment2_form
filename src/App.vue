        ...
        >
          <keep-alive>
            <component
              ...
              v-bind="getFormComponentBindingsByName(record.tab_name)"
      ...
      tabNameCompBindingsMap: {
        personal_info: {
          ...
          events: {
            ...
            onNoteInput: this.handleNoteInput
          }
        },
        orders: {
          compName: "FormOrders",
  ...
  methods: {
    ...
    getFormComponentBindingsByName(tabName) {
      ...
      if (tabName === "personal_info") {
        bindings.personalInfo = this.currentFormInfo.records.find(
          i => i.tab_name === "personal_info"
        ).data;
      } else if (tabName === "orders") {
        bindings.orders = this.currentFormInfo.records.find(
          i => i.tab_name === "orders"
        ).data;
      } else {
    ...
    handleNoFixedAddressClick() {
      this.currentFormInfoActiveTabData.address = "";
    },
    handleJobSelectChange(e) {
      const { value: selectedJob } = e.target;
      this.currentFormInfoActiveTabData.job = selectedJob;
    },
    handleNoteInput(e) {
      const { value: note } = e.target;
      this.currentFormInfoActiveTabData.note = note;
    },
